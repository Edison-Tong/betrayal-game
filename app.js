  ...
  if (!existingTile) {
    ...
    if (movingToTile.symbol !== "none" && movingToTile.symbol !== "event") {
      ...
      if (!availableCards[0]) {
        ...
        renderPlayerCards();
        if (movingToTile.symbol === "omen") {
          setTracker();
        }
      }
    } else if (movingToTile.symbol === "event") {
      let availableCards = cards.filter((card) => card.type === movingToTile.symbol);
      ...
      if (availableCards.length > 0) {
        ...
        activePlayer.cards.push(usedCard); // Add to player's cards
...
async function handleEndOfTurn() {
  if (canEndTurn) {
    ...
    // document.querySelector(".dice").innerHTML = `<image class="die" src="./images/dice/1.png"></image>
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      function raiseTrait(event) {
        ...
        if ((gainOrLose === "lose" && initialStats[trait] === playerStatsInfo[trait].index) || change === -amount) {
          return;
        }
      ...
      function lowerTrait(event) {
        ...
        if ((gainOrLose === "gain" && initialStats[trait] === playerStatsInfo[trait].index) || change === amount) {
          return;
        }
      ...
      function confirmTraits() {
        if (change === amount || change === -amount) {
          buttons.forEach((button) => {
            ...
            confirmTraitsBtn.removeEventListener("click", confirmTraits);
          });
          resolve({ change });
        }
        canEndTurn = true;
