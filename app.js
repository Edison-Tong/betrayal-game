  ...
  if (!existingTile) {
    ...
    if (movingToTile.symbol !== "none" && movingToTile.symbol !== "event") {
      ...
      if (availableCards.length > 0) {
        ...
        usedCard.effect(activePlayer); // Apply the card's effect
      }
    }
...
export function getPlayerChoice(options) {
  return new Promise((resolve) => {
    ...
    options.forEach((option) => {
      ...
      button.textContent = `Use ${option.charAt(0).toUpperCase() + option.slice(1)}`;
      button.onclick = () => {
